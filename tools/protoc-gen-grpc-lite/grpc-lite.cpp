...
#include "{3}"
    ...
    if (!file->package().empty()) 
    {
        ...
        std::size_t start = 0;
        for (auto end = file->package().find('.'); end != std::string::npos; end = file->package().find('.', start)) 
        {
    ...
    for (int i = 0; i < file->service_count(); i++) 
    {
        ...
        std::string tmp = fmt::format("using Service = grpcxx::service<\"{}.{}\"", file->package(), service->name());
        ...
        for (int j = 0; j < service->method_count(); j++) 
        {
            ...
            auto mapper = [&file](const google::protobuf::Descriptor *t) -> std::string
            {
                ...
                std::string result;
                std::size_t start = 0;
                for (auto end = name.find('.'); end != std::string::npos; end = name.find('.', start))
                {
...
struct ServiceImpl 
{
    ...
    if (!file->package().empty()) 
    {
        ...
        for (auto pos = file->package().rfind('.'); pos != std::string::npos; pos = file->package().rfind('.', end)) 
        {
            output +=
